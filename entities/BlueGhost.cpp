...
BlueGhost::BlueGhost() : BaseEntity(), _texture(), _speed(0.1f), _currentFrame(0)
{
    std::filesystem::path sourceDir = std::filesystem::path(__FILE__).parent_path().parent_path();
    std::filesystem::path assetPath = sourceDir / "assets" / "BlueGhost.png";
...
void BlueGhost::render(sf::RenderWindow& window)
{
    window.draw(_sprite);
}

void BlueGhost::handleInput()
{
